...
public class BookGoodsDao {
    ...
    public BookGoods findBookGoodsById(int id) {
        return em.find(BookGoods.class, id);
    }

    public List<BookGoods> findAllBookGoods(int bid) {
        TypedQuery<BookGoods> query;
        query = em.createQuery("select bg from BookGoods bg where bg.bookId = :bookid", BookGoods.class);
        query.setParameter("bookid", bid);

        List<BookGoods> results = query.getResultList();
